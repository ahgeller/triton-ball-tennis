...
try:
    ...
    try:
        ...
        from tennis_tracker.video_io import ThreadedFrameReader, VideoWriter, _PinnedFrameUploader, _cuda_frame_to_chw_f32
...
class ExperimentConfig:
    ...
    # ---- Performance / runtime mode (added) ----
    # fast_mode: drop the baseline comparison arm entirely (no 2nd connected-
    #   components pass, no baseline overlay), do the mask AND + scalar
    #   reductions on the GPU, and transfer only what the temporal path needs.
    #   Detection output (temporal_boost) is byte-for-byte identical to the
    #   non-fast path; only the experiment's comparison telemetry is dropped.
    fast_mode: bool = False
    # profile: accumulate per-stage wall-clock timings and print at the end.
    profile: bool = False
    # roi_radius: when > 0 AND a track exists, restrict the *expensive*
    #   downstream work (GPU->CPU transfer + connected components + per-blob
    #   analysis) to a (2*roi_radius)^2 box around the predicted ball position.
    #   The full-frame GPU motion/temporal compute still runs so the temporal
    #   stack stays in a consistent coordinate frame (see _process_cuda_roi).
    roi_radius: int = 0
    # roi_miss_reset: consecutive frames with no accepted blob before the ROI
    #   is dropped and a full-frame re-acquire pass runs.
    roi_miss_reset: int = 8
...
class TemporalMotionDetector:
    ...
    *postprocessed* (soft-opened + dilated) mask, not the raw per-pixel candidate.
    ...
    def __init__(self, cfg: ExperimentConfig):
        ...
        # ---- Runtime mode state (added) ----
        self.fast_mode: bool = bool(getattr(cfg, "fast_mode", False))
        self.roi_radius: int = max(int(getattr(cfg, "roi_radius", 0)), 0)
        self.roi_miss_reset: int = max(int(getattr(cfg, "roi_miss_reset", 8)), 1)
        # Simple constant-velocity track. Swap _predict_roi / _update_track for
        # your Kalman filter when you wire this into Triton's tracker.
        self._track_center: Optional[Tuple[float, float]] = None
        self._track_vel: Tuple[float, float] = (0.0, 0.0)
        self._roi_misses: int = 0
        # Per-stage timing accumulators (only touched when cfg.profile).
        self.profile: bool = bool(getattr(cfg, "profile", False))
    ...
    # ------------------------------------------------------------------
    # Profiling + ROI tracking helpers (added)
    # ------------------------------------------------------------------

    @contextmanager
    def _stage(self, name: str):
        """Time a pipeline stage. With CUDA, sync first so the timing is real."""
        if not self.profile:
            yield
            return
        if self.use_cuda:
    ...
    def profile_report(self) -> Dict[str, Dict[str, float]]:
        ...
        for name, total in sorted(self._stage_times.items(), key=lambda kv: -kv[1]):
            calls = max(self._stage_calls.get(name, 1), 1)
            out[name] = {"total_ms": total * 1e3, "per_frame_ms": total / calls * 1e3, "calls": calls}
        return out

    def _predict_roi(self, shape_hw: Tuple[int, int]) -> Optional[Tuple[int, int, int, int]]:
        """Return (x0, y0, x1, y1) box around the predicted ball, or None.

        Constant-velocity prediction. Replace with your Kalman predict step.
        """
        if self.roi_radius <= 0 or self._track_center is None:
            return None
        if self._roi_misses >= self.roi_miss_reset:
            return None
        h, w = shape_hw
        px = self._track_center[0] + self._track_vel[0]
        py = self._track_center[1] + self._track_vel[1]
        r = float(self.roi_radius)
        x0 = int(max(0, np.floor(px - r)))
        y0 = int(max(0, np.floor(py - r)))
        x1 = int(min(w, np.ceil(px + r)))
        y1 = int(min(h, np.ceil(py + r)))
        if x1 - x0 < 4 or y1 - y0 < 4:
            return None
        return x0, y0, x1, y1

    def _update_track(self, components: List[CandidateComponent], accepted_labels: set) -> None:
        """Update the constant-velocity track from accepted blobs.

        Picks the accepted blob nearest the prediction (or highest score on
        re-acquire). Replace with your Kalman update step.
        """
        if self.roi_radius <= 0:
            return
        accepted = [c for c in components if c.label in accepted_labels]
        if not accepted:
            self._roi_misses += 1
            return
        if self._track_center is not None:
            px = self._track_center[0] + self._track_vel[0]
            py = self._track_center[1] + self._track_vel[1]
            best = min(accepted, key=lambda c: (c.cx - px) ** 2 + (c.cy - py) ** 2)
        else:
            best = max(accepted, key=lambda c: (c.score_max, c.score_mean))
        if self._track_center is not None:
            self._track_vel = (best.cx - self._track_center[0], best.cy - self._track_center[1])
        self._track_center = (best.cx, best.cy)
        self._roi_misses = 0

    def process(self, frame_bgr: np.ndarray) -> Optional[FrameMotionResult]:
        with self._stage("stabilize"):
    ...
    def _process_cuda(self, frame_bgr: np.ndarray) -> Optional[FrameMotionResult]:
        ...
        with self._stage("motion_energy"):
        ...
        with self._stage("masks_gpu"):
            ...
            # Combine on the GPU (was a CPU cv2.bitwise_and on full frames).
            combined_t = baseline_raw_t & temporal_raw_t

        h, w = combined_t.shape
        reject_dep = bool(getattr(self.cfg, "reject_temporal_departures", False))
        need_signed = (signed_agg_t is not None) and (reject_dep or not self.fast_mode)

        # ROI gate: restrict transfer + CC to a box around the prediction.
        roi = self._predict_roi((h, w))
        if roi is not None:
            rx0, ry0, rx1, ry1 = roi
        else:
            rx0, ry0, rx1, ry1 = 0, 0, w, h

        with self._stage("gpu_reduce"):
            # Scalar reductions on the GPU, folded into one tiny transfer.
            stat_list = [
                temporal_score_t.mean(),
                temporal_score_t.max().to(torch.float32),
            ]
            if not self.fast_mode:
                stat_list.append(baseline_raw_t.sum().to(torch.float32))
            stats_cpu = torch.stack(stat_list).cpu().numpy()
            score_mean = float(stats_cpu[0])
            score_max = int(stats_cpu[1])
            baseline_raw_px = int(stats_cpu[2]) if not self.fast_mode else 0

        with self._stage("transfer"):
            # Slice on the GPU, transfer only the ROI box.
            combined_box = (combined_t[ry0:ry1, rx0:rx1].to(torch.uint8) * 255)
            score_box = temporal_score_t[ry0:ry1, rx0:rx1].clamp(0.0, 255.0).to(torch.uint8)
            if need_signed:
                pack_signed = signed_agg_t[ry0:ry1, rx0:rx1]
            stacked = torch.stack([combined_box.to(torch.float32),
                                   score_box.to(torch.float32)], dim=0).cpu().numpy()
            combined_box_np = stacked[0].astype(np.uint8)
            score_box_np = stacked[1].astype(np.uint8)
            signed_box_np = pack_signed.cpu().numpy().astype(np.float32) if need_signed else None
            if not self.fast_mode:
                baseline_box_np = (baseline_raw_t[ry0:ry1, rx0:rx1].to(torch.uint8) * 255).cpu().numpy()
            else:
                baseline_box_np = None

        # Re-embed ROI boxes into full-frame arrays so the rest of the
        # pipeline (border exclusion, overlays, summary) is unchanged.
        def _embed(box: Optional[np.ndarray], dtype) -> Optional[np.ndarray]:
            if box is None:
                return None
            full = np.zeros((h, w), dtype=dtype)
            full[ry0:ry1, rx0:rx1] = box
            return full

        if roi is not None:
            combined_raw = _embed(combined_box_np, np.uint8)
            temporal_score_u8 = _embed(score_box_np, np.uint8)
            signed_agg = _embed(signed_box_np, np.float32) if signed_box_np is not None else None
            baseline_raw = _embed(baseline_box_np, np.uint8) if baseline_box_np is not None else np.zeros((h, w), np.uint8)
        else:
            combined_raw = combined_box_np
            temporal_score_u8 = score_box_np
            signed_agg = signed_box_np
            baseline_raw = baseline_box_np if baseline_box_np is not None else np.zeros((h, w), np.uint8)

        with self._stage("border"):
            border = int(getattr(self.cfg, "border_exclude_px", 0))
            baseline_raw = _apply_border_exclusion(baseline_raw, border)
            combined_raw = _apply_border_exclusion(combined_raw, border)

        # Baseline arm (comparison telemetry only) — skipped in fast mode.
        with self._stage("cc_baseline"):
            if self.fast_mode:
                baseline_boost_result = CandidateMaskResult(mask=np.zeros((h, w), np.uint8), pixel_count=0, component_count=0)
            else:
                baseline_labels, baseline_components = _analyze_candidate_components(baseline_raw, self.cfg)
                baseline_boost_result = _mask_result_from_components(
                    baseline_labels, baseline_components, lambda comp: True
        ...
        with self._stage("cc_temporal"):
            combined_labels, combined_components = _analyze_candidate_components(
                combined_raw, self.cfg, score_u8=temporal_score_u8,
                signed_map=signed_agg if reject_dep else None,
                frame_bgr=frame_bgr,
            )
            accepted_labels, rejected_labels = _select_temporal_components(combined_components, self.cfg)
            temporal_boost_result = _mask_result_from_components(
        ...
        # Advance the (placeholder) constant-velocity track.
        self._update_track(combined_components, accepted_labels)
        ...
        return FrameMotionResult(
            ...
            temporal_rejected=temporal_rejected_result.mask,
            temporal_score_u8=temporal_score_u8,
            baseline_raw_px=baseline_raw_px,
            baseline_boost_px=baseline_boost_result.pixel_count,
            baseline_components=baseline_boost_result.component_count,
            temporal_raw_px=int(np.count_nonzero(combined_box_np)),
            temporal_boost_px=temporal_boost_px,
            temporal_components=temporal_boost_result.component_count,
            temporal_score_mean=score_mean,
            temporal_score_max=score_max,
            temporal_accept_score_u8=int(getattr(self.cfg, "temporal_accept_score_u8", 150)),
        )
...
def run_experiment(cfg: ExperimentConfig) -> Dict[str, object]:
    ...
    if bool(getattr(cfg, "profile", False)):
        report = detector.profile_report()
        summary["timing"]["stages"] = report
        print("[temporal-exp] per-stage timing (mean ms/frame):")
        for name, info in report.items():
            print(f"    {name:>14s}: {info['per_frame_ms']:7.3f} ms   (total {info['total_ms']/1e3:6.2f}s)")
...
def parse_args() -> ExperimentConfig:
    ...
    parser.add_argument("--write-debug-video", action="store_true",
                        help="Write the 2x2 comparison video (4x input resolution, slow). Off by default.")
    parser.add_argument("--fast", action="store_true",
                        help="Drop the baseline comparison arm; fuse AND+reductions on GPU. Same temporal detection.")
    parser.add_argument("--profile", action="store_true",
                        help="Print per-stage timing breakdown at the end.")
    parser.add_argument("--roi-radius", type=int, default=0,
                        help="If >0, gate transfer+CC to a (2r)^2 box around the predicted ball.")
    parser.add_argument("--roi-miss-reset", type=int, default=8,
                        help="Consecutive misses before dropping the ROI for a full-frame re-acquire.")
    ...
    return ExperimentConfig(
        ...
        write_debug_video=bool(args.write_debug_video),
        fast_mode=bool(args.fast),
        profile=bool(args.profile),
        roi_radius=max(0, int(args.roi_radius)),
        roi_miss_reset=max(1, int(args.roi_miss_reset)),
