...
| `grid` | 32 | 17,509 | amateur phone + TV, 1080p | a partial copy of GridTrackNet's public set (the full set is 100 matches / 48 GB and **is downloadable** — see §1). Several matches carry labels from a different cut of the video; `ft.py check --audit --fix` shifts or excludes them (`exclude.txt`) |
