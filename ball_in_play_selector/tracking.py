...
class _UltraTrackerInputDetections:
    """Minimal object compatible with Ultralytics BYTETracker/BOTSORT.update()."""
...
def _dets_to_ultra_tracker_input(
    frame_dets: List[Detection],
    cfg: SelectorConfig,
) -> _UltraTrackerInputDetections:
    n = len(frame_dets)
    if n == 0:
        return _UltraTrackerInputDetections(
    ...
    expand = max(1.0, float(getattr(cfg, "ultra_tracker_input_expand", 1.0)))
    min_side = max(0.0, float(getattr(cfg, "ultra_tracker_input_min_side_px", 0.0)))
    fw = max(2.0, float(cfg.width))
    fh = max(2.0, float(cfg.height))

    for i, d in enumerate(frame_dets):
        w = max(0.0, float(d.x2) - float(d.x1))
        h = max(0.0, float(d.y2) - float(d.y1))
        cx = float(d.cx)
        cy = float(d.cy)
        w = max(w * expand, min_side)
        h = max(h * expand, min_side)
        w = min(w, fw)
        h = min(h, fh)
        cx = float(np.clip(cx, 0.0, fw - 1.0))
        cy = float(np.clip(cy, 0.0, fh - 1.0))
    ...
    return _UltraTrackerInputDetections(xywh=xywh, conf=conf, cls=cls)

def _make_ultra_tracker_for_selector(cfg: SelectorConfig):
    from ultralytics.trackers.bot_sort import BOTSORT
    from ultralytics.trackers.byte_tracker import BYTETracker

    tracker_type = str(getattr(cfg, "ultra_tracker_name", "bytetrack")).lower()
    tracker_cfg: Dict[str, Any] = {
        "tracker_type": tracker_type,
        "track_high_thresh": float(getattr(cfg, "ultra_track_thresh", 0.12)),
        "track_low_thresh": float(getattr(cfg, "ultra_track_low_thresh", 0.02)),
        "new_track_thresh": float(getattr(cfg, "ultra_new_track_thresh", 0.50)),
        "track_buffer": int(getattr(cfg, "ultra_track_buffer", 240)),
        "match_thresh": float(getattr(cfg, "ultra_match_thresh", 0.75)),
        "fuse_score": True,
        "track_thresh": float(getattr(cfg, "ultra_track_thresh", 0.12)),
    }
    if tracker_type == "botsort":
        tracker_cfg.update(
            {
                "gmc_method": str(getattr(cfg, "ultra_gmc_method", "none")),
                "proximity_thresh": float(getattr(cfg, "ultra_proximity_thresh", 0.25)),
                "appearance_thresh": float(getattr(cfg, "ultra_appearance_thresh", 0.25)),
                "with_reid": bool(getattr(cfg, "ultra_enable_reid", False)),
            }
        )
    tracker_cls = BOTSORT if tracker_type == "botsort" else BYTETracker
    return tracker_cls(args=SimpleNamespace(**tracker_cfg), frame_rate=float(max(cfg.fps, 1.0)))

def _track_row_xyxy(row, frame_w: int, frame_h: int) -> Optional[Tuple[float, float, float, float]]:
    ...
    fw = max(2.0, float(frame_w))
    fh = max(2.0, float(frame_h))
    x1 = float(np.clip(x1, 0.0, fw - 1.0))
    y1 = float(np.clip(y1, 0.0, fh - 1.0))
    x2 = float(np.clip(x2, 0.0, fw - 1.0))
    y2 = float(np.clip(y2, 0.0, fh - 1.0))
    if x2 < x1:
        x1, x2 = x2, x1
    if y2 < y1:
        y1, y2 = y2, y1
...
def build_tracks_ultra(
    all_dets: List[List[Detection]],
    cfg: SelectorConfig
) -> List[Track]:
    """Build tracks with the same Ultralytics ByteTrack/BOTSort path as now_main_copy."""
    tracker = _make_ultra_tracker_for_selector(cfg)
    tracks_by_id: Dict[int, Track] = {}
    diag = _cfg_diag(cfg)
    inherit_gate = max(8.0, 0.03 * diag)

    for t, frame_dets in enumerate(all_dets):
        tracker_input = _dets_to_ultra_tracker_input(frame_dets, cfg)
        tracker_img = None  # selector Step-1 is detection-only; no frame needed for bytetrack
        rows = tracker.update(tracker_input, tracker_img)
        if rows is None or len(rows) == 0:
            continue

        best_row_by_tid: Dict[int, Tuple[float, Tuple[float, float, float, float]]] = {}
        for row in rows:
            if row is None or len(row) < 5:
                continue
            tid = int(round(float(row[4])))
            score = float(row[5]) if len(row) > 5 else 0.0
            xyxy = _track_row_xyxy(row, cfg.width, cfg.height)
            if xyxy is None:
                continue
            prev = best_row_by_tid.get(tid)
            if prev is None or score > prev[0]:
                best_row_by_tid[tid] = (score, xyxy)

        for tid, (trk_score, xyxy) in best_row_by_tid.items():
            x1, y1, x2, y2 = xyxy
            cx = 0.5 * (x1 + x2)
            cy = 0.5 * (y1 + y2)
            area = max((x2 - x1) * (y2 - y1), 1.0)
            obs_conf = float(max(0.0, trk_score))
            on_motion = False
            obs_x1, obs_y1, obs_x2, obs_y2 = float(x1), float(y1), float(x2), float(y2)

            if frame_dets:
                nearest = min(frame_dets, key=lambda d: (float(d.cx) - cx) ** 2 + (float(d.cy) - cy) ** 2)
                dxy = math.sqrt((float(nearest.cx) - cx) ** 2 + (float(nearest.cy) - cy) ** 2)
                if dxy <= inherit_gate:
                    obs_conf = float(nearest.conf)
                    on_motion = bool(nearest.on_motion)
                    # Preserve original YOLO box footprint for rendering/size-sensitive logic.
                    obs_x1 = float(nearest.x1)
                    obs_y1 = float(nearest.y1)
                    obs_x2 = float(nearest.x2)
                    obs_y2 = float(nearest.y2)
                    area = max((obs_x2 - obs_x1) * (obs_y2 - obs_y1), 1.0)

            det_obs = Detection(
                frame=t,
                cx=float(cx), cy=float(cy),
                x1=obs_x1, y1=obs_y1, x2=obs_x2, y2=obs_y2,
                conf=obs_conf, area=float(area), on_motion=on_motion,
            )

            trk = tracks_by_id.get(tid)
            if trk is None:
                trk = Track(track_id=int(tid), cfg=cfg)
                tracks_by_id[tid] = trk
            if trk.observations and trk.observations[-1].frame == t:
                continue
            trk.update(det_obs)
