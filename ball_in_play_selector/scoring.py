...
def _select_timeline_chain(
    ...
    for j in range(n):
        ...
        for i in range(j):
            ...
            # On tiny time gaps, reject implausible big court jumps.
            if gap <= small_gap and dfrac > jump_reject:
                continue
            # Also reject large jumps on medium gaps before transition scoring.
            if gap <= medium_gap and dfrac > (jump_reject + 0.08):
                continue
            # Global safety cap: never stitch extremely distant hops.
            if dfrac > 0.70:
                continue
